...
export async function processAndUpsertInvoice(
  ...
  providerName: string = 'zoho_books'
): Promise<void> {
  ...
  if (zohoInvoice.customer_id) {
    ...
    if (customers.length > 0) {
      ...
      console.log(`[Sync Engine] Found customer ${customerId} for ${providerName} customer ${zohoInvoice.customer_id}`);
    } else {
      // Customer not found - log warning and set customerId to null (Requirement FR-7)
      console.warn(`[Sync Engine] Customer not found in cache for ${providerName} customer ${zohoInvoice.customer_id} (invoice ${zohoInvoice.invoice_id})`);
    }
  }
  ...
  if (existingInvoices.length === 0) {
    ...
    await createRemindersForInvoice(newInvoiceId, userId, dueDate, settings, providerName);
  } else {
    ...
    if (existingInvoice.syncHash !== syncHash) {
      ...
      await handleInvoiceChanges(existingInvoice.id, changes, dueDate, userId, settings, providerName);
    } else {
...
export async function handleInvoiceChanges(
  ...
  if (changes.dueDateChanged) {
    ...
    await createRemindersForInvoice(invoiceId, userId, newDueDate, settings, providerName);
  }
...
export async function createRemindersForInvoice(
  ...
  dueDate: Date,
  settings: ReminderSettings,
  _providerName: string = 'zoho_books'
): Promise<void> {
...
export async function syncInvoicesForUser(
  ...
  console.log(`[Sync Engine] Starting sync for user ${userId}, organization ${organizationId} (provider: ${providerName})`);
  ...
  try {
    ...
    console.log(`[Sync Engine] Fetching invoices from ${providerName}...`);
